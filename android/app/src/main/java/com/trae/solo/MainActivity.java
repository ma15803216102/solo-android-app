            ...
            if (bridge != null) {
                ...
                if (webView != null) {
                    String js = "window.__SOLO_APP_VERSION__=" + "'" + getAppVersionName() + "'" + ";" +
                             ...
                             "s.innerHTML += `#t-fab{display:block !important;width:56px !important;height:56px !important;--dir:-1;}#t-fab.dir-down{--dir:1;}#t-fab #t-main{position:absolute !important;left:50% !important;top:0 !important;transform:translateX(-50%) !important;}#t-fab .t-sub{position:absolute !important;left:50% !important;top:4px !important;display:none !important;opacity:0 !important;transform:translate(-50%,0) scale(.8) !important;pointer-events:none !important;}#t-fab.exp .t-sub{display:flex !important;opacity:1 !important;transform:translate(-50%, calc(var(--dir) * var(--i) * 60px)) scale(1) !important;pointer-events:auto !important;}`;" +
                             ...
                             "document.getElementById('t-back').style.setProperty('--i','1');" +
                             "document.getElementById('t-ref').style.setProperty('--i','2');" +
                             "document.getElementById('t-upd').style.setProperty('--i','3');" +
                             "document.getElementById('t-vc-toggle').style.setProperty('--i','4');" +
                             "document.getElementById('t-auto-toggle').style.setProperty('--i','5');" +
                             ...
                             "};" +
                             "let lastTouchTs=0;" +
                             "const setDir=()=>{try{const r=c.getBoundingClientRect();const need=60*5;const spaceB=window.innerHeight-r.bottom;const spaceA=r.top;const down=spaceB>=need&&spaceB>=spaceA;c.classList.toggle('dir-down',down);}catch(e){c.classList.remove('dir-down');}};" +
                             "const toggleExp=()=>{if(!c.classList.contains('exp')) setDir();c.classList.toggle('exp');if(c.classList.contains('exp')) clearTimeout(ht); else resT();};" +
