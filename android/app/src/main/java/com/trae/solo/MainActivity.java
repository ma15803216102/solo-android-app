            ...
            if (bridge != null) {
                ...
                if (webView != null) {
                    String js = "window.__SOLO_APP_VERSION__=" + "'" + appVersionName + "'" + ";" +
                             ...
                             "window._traeCheckUpdate=async(manual)=>{try{if(manual) setUpdateStatus('正在检查更新...');const r=await fetch('https://api.github.com/repos/ma15803216102/solo-android-app/releases/latest',{cache:'no-store'});if(!r.ok){setUpdateStatus('检查更新失败：网络或接口异常');return;}const j=await r.json();const tag=j.tag_name||'';const assets=Array.isArray(j.assets)?j.assets:[];const isCN=(location.host||'').includes('trae.cn');const key=isCN?'TRAE-SOLO-CN-':'TRAE-SOLO-Global-';const apk=assets.find(a=>a&&typeof a.name==='string'&&a.name.includes(key)&&a.name.endsWith('.apk'));const url=apk?apk.browser_download_url:null;window._traeLatestDownloadUrl=url;const cur=window.__SOLO_APP_VERSION__||'';const curN=parseInt((cur.split('.').pop()||'0'),10);const tagN=parseInt((tag.replace(/^v/,'').split('.').pop()||'0'),10);if(tagN>curN&&url){setUpdateStatus('发现新版本：'+tag+'\\n点击“下载更新”获取安装包。');document.getElementById('t-update-download').style.display='inline-block';}else{setUpdateStatus('当前已是最新版本。\\n最新版本：'+(tag||'未知'));document.getElementById('t-update-download').style.display='none';}}catch(e){setUpdateStatus('检查更新失败：'+(e&&e.message?e.message:'未知错误'));}};\n" +
